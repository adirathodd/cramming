...
def train(model, dataset, tc: TrainingConfig):
    ...
    last_test_accuracy = math.nan
    last_test_prefix_accuracy = math.nan
    ...
    for epoch in pbar:
        ...
        should_eval = ((epoch + 1) % tc.eval_every == 0) or (epoch == tc.epochs - 1)
        if should_eval:
            if is_variable_length:
                test_loss, test_accuracy, test_prefix_accuracy = eval_step_multi_length(
                    model, test_splits_dev, tc.training_target, tc.eval_batch_size
                )
            else:
                test_loss, test_accuracy, test_prefix_accuracy = eval_step(
                    model, test_data_dev, test_labels_dev, tc.training_target, tc.eval_batch_size
                )
            last_test_loss = test_loss
            last_test_accuracy = test_accuracy
            last_test_prefix_accuracy = test_prefix_accuracy
        else:
            test_loss = last_test_loss
            test_accuracy = last_test_accuracy
            test_prefix_accuracy = last_test_prefix_accuracy
        ...
        if should_eval or not math.isnan(last_test_loss):
            postfix['TsL'] = f'{test_loss:.3f}'
            postfix['TsA'] = f'{test_accuracy:.2f}%'
        if tc.training_target == 'seq_cot' and train_prefix_accuracy is not None:
            postfix['TrPA'] = f'{train_prefix_accuracy:.2f}%'
            if should_eval or not math.isnan(last_test_prefix_accuracy):
                postfix['TsPA'] = f'{test_prefix_accuracy:.2f}%'
        if not should_eval:
            postfix['Eval'] = f'every {tc.eval_every}'
